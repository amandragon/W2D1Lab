...
describe SortedArray do
  ...
  describe '#first_larger_index' do
    before do
      @source = [2,3,4,7,9]
      @sorted_array = SortedArray.new #new instance of sorted array
      @sorted_array.internal_arr = @source #the internal array=source
    end

    it 'gives 0 for an empty array' do 
      @sorted_array.internal_arr = [] #empty array
      @sorted_array.first_larger_index(4).should == 0 #why 4?
    end
